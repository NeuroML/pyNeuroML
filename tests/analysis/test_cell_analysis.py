...
import neuroml
from pyneuroml.analysis.NML2CellAnalysis import (
    analyse_point_neuron,
...
class TestCellAnalysis(unittest.TestCase):
    ...
    def test_analyse_multi_compartmental_neuron(self):
        ...
        cell_doc = neuroml.loaders.read_neuroml2_file(
            dirname + "/../plot/test.cell.nml", include_includes=True
        )
        ...
        for x in ["leak_chan", "HCNolm", "Kdrfast", "KvAolm", "Nav"]:
            includes.append(
                cell_doc.component_factory(
                    "IncludeType", href=f"tests/plot/olm-example/{x}.channel.nml"
                )
            )

        analyse_multi_compartmental_neuron(
            cell,
            output_segment_ids=["0", "3"],
            includes=includes,
            min_amplitude=0.01,
            max_amplitude=0.20,
            num_amplitudes=5,
