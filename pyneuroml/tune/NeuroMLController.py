...
class NeuroMLController():
    ...
    def run(self, candidates, parameters):
        ...
        if self.num_parallel_evaluations == 1:
            ...
            logger.info('Running %i candidates across %i local processes' % (len(candidates), job_server.get_ncpus()))
            jobs = []
            ...
            for job_i in range(len(jobs)):
                job = jobs[job_i]
                logger.info("Checking parallel job %i/%i; set running so far: %i" % (job_i, len(jobs), self.count))
                t, v = job()
                traces.append([t, v])
        ...
        logger.info('Ran %i candidates in %s seconds (~%ss per job)' % (len(candidates), tot, tot / len(candidates)))
        return traces
...
def run_individual(sim_var,
    ...
    for var_name in sim_var.keys():
        ...
        for individual_var_name in individual_var_names:
            ...
            logger.info('  Changing value of %s (%s) in %s (%s) to: %s %s' % (variable, id2, type, id1, value, units))
            ...
            if type == 'channel':
                ...
                if channel:
                    ...
                    logger.info('Could not find channel with id %s from expression: %s' % (id1, individual_var_name))
                    exit()
            elif type == 'cell':
                cell = None
                ...
                if variable == 'channelDensity':
                    ...
                    for sc in cell.biophysical_properties.membrane_properties.specific_capacitances:
                        if (sc.segment_groups is None and id2 == 'all') or sc.segment_groups == id2:
                            specCap = sc
                    ...
                    for rs in cell.biophysical_properties.intracellular_properties.resistivities:
                        if (rs.segment_groups is None and id2 == 'all') or rs.segment_groups == id2:
                            resistivity = rs
                    resistivity.value = '%s %s' % (value, units)
                else:
                    logger.info('Unknown variable (%s) in variable expression: %s' % (variable, individual_var_name))
                    exit()
                ...
                logger.info('Unknown type (%s) in variable expression: %s' % (type, individual_var_name))

    new_neuroml_file = '%s/%s' % (generate_dir, os.path.basename(neuroml_file))
    if new_neuroml_file == neuroml_file:
        logger.info('Cannot use a directory for generating into (%s) which is the same location of the NeuroML file (%s)!' % \
    ...
    for include in still_included:
        ...
        logger.info("Copying non included file %s to %s (%s) beside %s" % (inc_loc, generate_dir, os.path.abspath(generate_dir), new_neuroml_file))
