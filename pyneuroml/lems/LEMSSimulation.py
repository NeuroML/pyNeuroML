...
from pyneuroml.utils import read_lems_file
from pyneuroml.utils import get_next_hex_color
import random
...
class LEMSSimulation:
    ...
    def include_neuroml2_file(
        self, nml2_file_name, include_included=True, relative_to_dir="."
    ):
    ...
    def add_line_to_display(
        self, display_id, line_id, quantity, scale=1, color=None, timeScale="1ms"
    ):
        disp = None
    ...
    def add_selection_to_event_output_file(
        self, event_output_file_id, event_id, select, event_port
    ):
        eof = None
