...
def run_jneuroml_with_realtime_output(pre_args,
    ...
    try:
        command = 'java -Xmx%s %s -jar  "%s" %s "%s" %s' % \
              (max_memory, pre_jar, jar_path, pre_args, target_file, post_args)
        command_success = execute_command_in_dir_with_realtime_output(command, exec_in_dir, verbose=verbose,
                                        prefix = ' jNeuroML >>  ')
    ...
    if p.returncode != 0:
        ...
        try:
            with p.stderr as stderr:
                print_comment_v('*** Error Output: \n%s'%(stderr.read().decode('utf-8')))
        except Exception as e:
            print_comment("Unable to print stderr:", e)
            traceback.print_exc()
    else:
        try:
            p.stderr.close()
        except Exception:
            print("Unable to close error output stream:")
            traceback.print_exc()
        try:
            p.stdout.close()
        except Exception:
            print("Unable to close output stream:")
            traceback.print_exc()
...
def generate_plot(xvalues,
    ...
    for i in range(len(xvalues)):
        ...
        if colors:
            plt.plot(xvalues[i], yvalues[i], color=colors[i], marker=marker, markersize=markersize, linestyle=linestyle, linewidth=linewidth, label=label)
        else:
            plt.plot(xvalues[i], yvalues[i], marker=marker, markersize=markersize, linestyle=linestyle, linewidth=linewidth, label=label)
